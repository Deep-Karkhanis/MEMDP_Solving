...
def diagDrifts(maze,i,j,direction):
    """
    Returns how many of the 2 possible cross blocks can the robot drift into.
    It can't drift there if it is a wall
    Assumption: Robot is never on the edge of the maze. There will be an outermost ring of walls in the maze
    """
    if(direction.__eq__('N')):
        return int(maze[i-1][j-1] != '1'), int(maze[i-1][j+1] != '1')
    if(direction.__eq__('E')):
        return int(maze[i-1][j+1] != '1'), int(maze[i+1][j+1] != '1')
    if(direction.__eq__('W')):
        return int(maze[i+1][j-1] != '1'), int(maze[i-1][j-1] != '1')
...
if __name__ == "__main__":
    ...
    # Hyperparameters
    actions = ['F','L','R']
    failures = []# Probability of staying still after action forward, left and right respectively.
    ...
    if args.fin is not None:
        ...
        with open(args.fin, 'r') as fIn:
            for line in fIn.read().splitlines():
                if failure_info:
                    if nxt_info_F:
                        failures.append([float(x) for x in line.split()])
                        nxt_info_F = 0
                        nxt_info_L = 1
                    elif nxt_info_L:
                        failures.append([float(x) for x in line.split()])
        ...
        while (len(mazes) != len(failures[0])):
            mazes.append(mazes[0])
        # if len(maze) > 1:
        #     mazes.append(maze)
        #     x1, x2, y1, y2 = mazeBoundaries(maze)
        #     min_x = min(min_x, x1); max_x = max(max_x, x2);
    ...
    for e, maze in enumerate(mazes):
        ...
        if args.rdf:
            ...
            f_summary.write("\nFailure in going forward\n")
            f_summary.write("%s\n" % (' '.join("%.3f" % x for x in failures[0])))
            f_summary.write("\nFailure in going left\n")
            f_summary.write("%s\n" % (' '.join("%.3f" % x for x in failures[1])))
            f_summary.write("\nFailure in going right\n")
            f_summary.write("%s\n" % (' '.join("%.3f" % x for x in failures[2])))
                ...
                for orient in ['N','E','S','W']:
                    ...
                    if element == 'x':
                        for action in actions:
                            ...
                            # f_rewards.write("%s %s %s %f\n" % (current_state, action, 'T', trap_penalty))
                    # G.O.A.L
                    elif element == 'g':
                        for action in actions:
                    ...
                    # E.L.S.E
                        ...
                        # target, fail = ('T', 1.0 - wall_failure) if isWall(i, j, orient) else 

                        fail_to_diag = failures[0][e]
                        fail_into_wall = wall_failure
                        left_diag,right_diag = diagDrifts(maze,i,j,orient)

                        if left_diag == 1:
                            left_orient = turnLeft(orient)
                            left_i,left_j = (i + changeMap[orient][0] + changeMap[left_orient][0], j + changeMap[orient][1] + changeMap[left_orient][1])
                            left_drift_state = "%dx%dx%s" % (left_i, left_j, orient)
                            f_transitions.write("%s %s %s %f\n" % (current_state, 'F', left_drift_state, fail_to_diag))
                            # f_rewards.write("%s %s %s %f\n" % (current_state, 'F', left_drift_state, forward_cost))

                        if right_diag == 1:
                            right_orient = turnRight(orient)
                            right_i,right_j = (i + changeMap[orient][0] + changeMap[right_orient][0], j + changeMap[orient][1] + changeMap[right_orient][1])
                            right_drift_state = "%dx%dx%s" % (right_i, right_j, orient)
                            f_transitions.write("%s %s %s %f\n" % (current_state, 'F', right_drift_state, fail_to_diag))
                            # f_rewards.write("%s %s %s %f\n" % (current_state, 'F', right_drift_state, forward_cost))

                        #if isWall(i, j, orient):
                            # f_transitions.write("%s %s %s %f\n" % (current_state, 'F', 'T', fail_into_wall))
                            # "Not taking wall failure"

                        # print ("\n\nCheck eror\n")
                        # print("%f" % fail_to_diag)
                        
                        # success_prob = 1.0 - (((left_diag+right_diag)*fail_to_diag) + ( (int(isWall(i,j,orient))) * fail_into_wall))
                        success_prob = 1.0 - ((left_diag+right_diag)*fail_to_diag)
                        
                        success_state = ("%dx%dx%s" % (i + changeMap[orient][0], j + changeMap[orient][1],orient)) if not isWall(i, j, orient) else current_state                        

                        f_transitions.write("%s %s %s %f\n" % (current_state, 'F', success_state, success_prob))
                        # f_rewards.write("%s %s %s %f\n" % (current_state, 'F', success_state, forward_cost))
                                       
                        # Turn left
                        target = "%dx%dx%s" % (i, j, left[orient]);
                        # f_transitions.write("%s %s %s %f\n" % (current_state, 'L', target, 1.0 - failures[e][1]))
                        # f_transitions.write("%s %s %s %f\n" % (current_state, 'L', current_state, failures[e][1]))
                        f_transitions.write("%s %s %s %f\n" % (current_state, 'L', target, 1.0))
                        # f_rewards.write("%s %s %s %f\n" % (current_state, 'L', target, turn_cost))
                    

                        # Turn right
                        target = "%dx%dx%s" % (i, j, right[orient]);
                        # f_transitions.write("%s %s %s %f\n" % (current_state, 'R', target, 1.0 - failures[e][2]))
                        # f_transitions.write("%s %s %s %f\n" % (current_state, 'R', current_state, failures[e][2]))
                        f_transitions.write("%s %s %s %f\n" % (current_state, 'R', target, 1.0))
                        # f_rewards.write("%s %s %s %f\n" % (current_state, 'R', target, turn_cost))
